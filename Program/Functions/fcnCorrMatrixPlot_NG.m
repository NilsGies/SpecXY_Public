...
fig=figure('Position',[.05*scrsz(3) .05*scrsz(4) .9*scrsz(3) .87*scrsz(4)]);
...
for i = 1:n-1      % Loop for rows
 if i > counter
  for j = 2:n      % Loop for columns
   if j > counter+1
    try
    rows2keep=not((isnan(A(:,i)) | isnan(A(:,j))));

    X = A(rows2keep,i);
    Y = A(rows2keep,j);       % Data to be plotted

    if exist('classes','var') && not(isempty(classes))
        C=classes(rows2keep);
    end
    ...
    if exist('classes','var') && not(isempty(classes))
        %  scatter(X,Y,50,C,'filled','MarkerEdgeColor','k')
        if exist('marker','var')
            if strcmp(marker,'+') || strcmp(marker,'*')  || strcmp(marker,'x') || strcmp(marker,'_')|| strcmp(marker,'|')
                scatter(X,Y,symbolsize*10,C,'Marker',marker,'LineWidth',2)
            elseif strcmp(marker,'.')

               % scatter(X,Y,symbolsize,C,'Marker',marker,'LineWidth',2)
              %  scatter(X,Y,1,C,'Marker',marker,'AlphaData',repmat(0.1,size(Y,1)),1)
                scatter(X,Y,1,C,'Marker',marker)

            else
                scatter(X,Y,symbolsize*5,C,'filled','Marker',marker)
            end
        else
            scatter(X,Y,symbolsize*4,C,'filled')
...
% 
% 
% function [fig] = fcnCorrMatrixPlot_NG(A, VarName, caption,classes,cmap)
% % This function produces a correlation matrix scatterplot with
% % least-squared fitted lines.
% % Input A is a mxn matrix (m: # of observations, n: # of variables).
% % VarName is a cell array with variable names correspond to columns of A.
